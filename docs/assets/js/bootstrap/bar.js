...
KISSY.add('bootstrap/bar',function(S){
	...
	/**
	* 翻页时触发
	* @name bar.PaggingBar#beforepagechange
	* @event  
	* @param {event} e  事件对象
	...
	*/
	...
	/** @lends bar.PaggingBar.prototype */	
	{
		...
		_getPageInfoConfig : function(){
			var temp = ['<span class="', CLS_IEXT_CONTAINER, ' ', CLS_TEXT_PAGE, '">共 0 页</span><span class="', CLS_IEXT_CONTAINER, '">第</span><span class="', CLS_INPUT_CONTAINER, '"><input type="text" autocomplete="off" class="', CLS_ITEM_INPUT, '" size="20" name="inputItem"> 页</span>'].join('');
	...
	*/
	...
	bar.TextBarItem = textBarItem;
	bar.LinkBarItem = linkBarItem;
	bar.CustomBarItem = customBarItem;
	bar.SeperatorBarItem = seperatorBarItem;
	return bar;
}, {requires : ['./bar.css']});